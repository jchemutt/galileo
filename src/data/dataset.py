...
class CustomPatchDataset(Dataset):
    def __init__(self, root_dir, split="train", transform=None, label2idx=None):
        """
        Args:
            root_dir (str): Root directory containing 'train', 'val', 'test' subfolders.
            split (str): One of 'train', 'val', 'test'.
            transform (callable, optional): Optional transform to apply to each image tensor.
            label2idx (dict, optional): Shared label-to-index mapping (used for val/test).
        """
        ...
        # Load labels for current split
        labels = [np.load(os.path.join(self.data_dir, f))["label"].item() for f in self.files]

        # Use shared label2idx if provided (for val/test)
        if label2idx is not None:
            self.label2idx = label2idx
        else:
            # Create label2idx from current split (for training only)
    ...
    def __getitem__(self, idx):
        ...
        image = data["image"].astype(np.float32)  # shape: (72, H, W)
        label_name = data["label"].item()

        if label_name not in self.label2idx:
            raise ValueError(f"Label '{label_name}' not found in label2idx mapping.")

        label = self.label2idx[label_name]

        # Normalize Sentinel-2 bands (B2–B12) per time step
        for t in range(6):
            base = t * 12
            image[base + 2 : base + 12] /= 10000.0
...
class PixelwisePatchDataset(Dataset):
    def __init__(self, root_dir, split="train", transform=None, ignore_index=255, valid_labels={0, 1, 2, 3}):
        ...
        Args:
            root_dir (str): Root directory containing 'train', 'val', 'test' subfolders with .npz files.
            split (str): One of ['train', 'val', 'test'].
    ...
    def __getitem__(self, idx):
        ...
        image = data["image"].astype(np.float32)  # shape: (C, H, W)
        raw_mask = data["mask"].astype(np.int64)  # shape: (H, W)

        # Convert raw labels to class indices, mask unknowns with ignore_index
        label_mask = np.full_like(raw_mask, fill_value=self.ignore_index)
